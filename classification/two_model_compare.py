...
ratios = ["BTC", "ETH", "LTC"]
for ratio in ratios:
    df = get_daily_data(api_key, ratio)  # read in specific file

    # rename volume and close to include the ticker so we can still which close/volume is which:
    df.rename(columns={"4a. close (USD)": f"{ratio}_close", "5. volume": f"{ratio}_volume"}, inplace=True)
...
#%%
model1=Sequential()
model1.add(LSTM(units=100,return_sequences=True,input_shape=(train_x.shape[1:])))
...
model1.fit(train_x,train_y,epochs=100,batch_size=32,validation_data=(validation_x, validation_y),callbacks=[tensorboard])
model1.save("models/{}".format(NAME))
#%%
# Train model
...
#%%
y_pred = model1.predict_classes(validation_x)
from sklearn.metrics import confusion_matrix
cm = confusion_matrix(validation_y,y_pred)
print(cm)

#%%
y_pred = model.predict_classes(validation_x)
from sklearn.metrics import confusion_matrix
cm = confusion_matrix(validation_y,y_pred)
print(cm)